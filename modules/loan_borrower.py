...
class OkxLoanBorrower(AbstractLoanBorrower):
    def __init__(self, exchange:ccxt.okx):
        self.exchange = exchange
    
    def try_loan_borrow(self, symbol):
        inst_id = symbol + "-USDT" #Instrument ID, e.g. BTC-USDT
        ccy = symbol #Loan currency, e.g. BTC
        # collateral_amount = self.calcaulte_collateral(symbol)
        params_loan_borrow = {
            "instId":inst_id,
            "ccy":symbol,
        ...
        print("OKX loan" + str(params_loan_borrow))
        result_message = self.exchange.privatePostAccountQuickMarginBorrowRepay(params=params_loan_borrow)
        message_sender.send_telegram_message(result_message)
        #self.add_colleteral(symbol, collateral_amount)
        return(result_message)
    ...
    def retrieve_collateral_max_limit(self, symbol):
        ...
        loan_data = self.exchange.privateGetAccountMaxLoan(params=params_max_loan)
        print(loan_data['data'])
        #loan_max_limit = loan_data['data'][0]['maxLoan']
